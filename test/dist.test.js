...
if (!process.env.NPM_CONFIG_TEST_DIST) {
	...
	const tName = 'CJS <=> ESM';
	if (vNodeJSMajor < 12) {
		test.skip(tName + ' (requires Node-v12+)', () => void 0);
	} else {
		test(tName, async (t) => {
			// eslint-disable-next-line security/detect-non-literal-require , security-node/detect-non-literal-require-calls
	...
	test("package 'exports' consistency", (t) => {
		...
		if (pkg.exports['./esm']) {
			...
			// const extension = path.extname(pathImport);
			// const basename = path.basename(pathImport, extension);
			// const dirname = path.dirname(pathImport);
			// t.is(pkg.exports['./esm'].types, path.posix.join(dirname, basename) + '.d.ts');
